...
import { setDevice, setError, updateDevice } from '@/store/slices/deviceSlice';
import { RootState } from '@/store/store';
import { Contractor } from '@/types/content';
import { Device, Entity } from '@/types/devices';
...
export function useAddDevice() {
  ...
  const handleInputChange = useCallback(
    <T extends string | Entity | Contractor>(field: keyof Device, value: T) => {
      ...
      // const inputValue = typeof value === 'string' ? value : isEntity(value) ? value.slug : '';
  ...
  const resetModelData = useCallback(() => {
    // dispatch(updateDevice({ field: "modelName", value: "" })); todo  проверить что это
