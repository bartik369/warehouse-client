...
import { Submenu } from '@/widgets/sidebar/model/types';
...
const SubMenu = ({ item, open, title }: SubMenuProps) => {
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.subTitle}>{!open && title}</div>
      <ul className={styles.submenu}>
        {item.subMenu.map((subItem: Submenu) => (
          <li
            key={subItem.id}
            className={location.pathname.includes(subItem.path) ? styles.active : ''}
