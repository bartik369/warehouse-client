...
export const useDeviceFilters = () => {
  ...
  const getUniqueOptions = (key: keyof IDeviceFilters):CheckedDeviceOptions[] => {
    ...
    const optionsMap: Record<keyof IDeviceFilters, any[]> = {
      manufacturer: devices.map((device: IFilteredDevicesFromBack, index: number) => ({
        id: index + 1,
        name: device.model.manufacturer.name,
        type: device.model.manufacturer.slug,
        value: device.model.manufacturer.slug,
      })),
      isFunctional: devices.map((device: IFilteredDevicesFromBack, index: number) => ({
        id: index + 1000,
      ...
      memorySize: devices.map((device: IFilteredDevicesFromBack, index: number) => ({
        id: index + 3000,
        name: String(device.memorySize),
        type: 'memorySize',
        value: device.memorySize,
      })),
      screenSize: devices.map((device: IFilteredDevicesFromBack, index: number) => ({
        id: index + 4000,
      ...
      warehouse: devices.map((device: IFilteredDevicesFromBack, index: number) => ({
        id: index + 6000,
